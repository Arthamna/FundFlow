...
// --- 1. DATA INITIALIZATION (MOCK DATABASE) ---
const initData = () => {
        ...
        const campaigns = [
            {
                ...
                target: 50000000,
                collected: 30000000,
                image: 'https://images.unsplash.com/photo-1541976844346-f18aeac57230?ixlib=rb-1.2.1&auto=format&fit=crop&w=800&q=80',
...
const register = (e) => {
    ...
    if (password !== confirmPass) return alert('Password tidak cocok!');

    const users = JSON.parse(localStorage.getItem('users'));
    if (users.find(u => u.email === email)) return alert('Email sudah terdaftar!');

    users.push({ email, password, role: 'user', name: document.getElementById('regUsername').value });
    localStorage.setItem('users', JSON.stringify(users));
            ...
            verifiedContainer.innerHTML += `
                <div class="col-md-4 mb-4">
                    <div class="card h-100">
                        <img src="${c.image}" class="card-img-top" style="height: 200px; object-fit: cover;" alt="...">
                        <div class="card-body">
...
const loadDonationDetail = () => {
    ...
    document.getElementById('detailImage').src = campaign.image;
